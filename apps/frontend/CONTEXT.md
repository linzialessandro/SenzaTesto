...
- **URL Filters**: Query-string state (`q`, `topic`, `year`, `difficulty`, `exercise`, `mode`, `size`) that drives search/practice and is shareable on reload.
- **Practice Session**: Guided 5–10 exercise loop (`mode=practice`) with self-assessment before solution reveal; progress in `localStorage` only.
...
- **Practice**: `components/practice/PracticeSession.tsx` + `lib/practiceProgress.ts` (pick/shuffle, recommendations, local history).
...
- **Component Structure**: `layout` (NavBar), `home` (HeroSection, SearchSection, CollectionsGrid, ExercisesGrid, HomeClient), `practice`, `modals`, share widgets.

## Related knowledge

- Discovery & permalinks: `docs/knowledge/architecture/exercise-discovery.md`
- Practice session MVP: `docs/knowledge/architecture/practice-session.md`
